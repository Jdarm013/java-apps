...
public class CSVReaderWriter<T extends CSVTemplate & Comparable<T>> { // Note the Comparable constraint
     ...
     * This is optional.  If not sorted, it would load into the priority queue just fine
     * but is more humanly readable this waay.
     */
    ...
    /**
     * Saves the data in a unsorted fashion, regardless of how the
     * Collection (like PriorityQueue) stores it internally.
     */
    public void saveToCSVDefaultSort(String header) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(filePath))) {
            if (header != null) pw.println(header);
